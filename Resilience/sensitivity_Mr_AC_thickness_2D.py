"""
2D sensitivity study: pavement design life over subgrade Mr x AC thickness.

Sweeps subgrade resilient modulus (Mr) and AC thickness together on a
grid, holding base thickness and all moduli (other than Mr) fixed,
computing PyMastic strains and design life at every combination. Unlike
the AC-modulus x base-modulus and AC-thickness x base-thickness 2D
sweeps (two structural variables), this one crosses a seasonal/
environmental variable (Mr) with a structural design variable (H_AC) -
directly showing how much AC thickness buys back design life lost to
seasonal subgrade softening, generalizing sensitivity_subgrade_Mr.py and
sensitivity_AC_thickness.py.

Outputs
-------
Resilience/results/Mr_AC_thickness_2D.csv   (long format, one row per grid point)
Resilience/results/Mr_AC_thickness_2D.png   (2-panel: governance map + design-life heatmap)
"""
import os
import sys
import csv
...
# --- Fixed base thickness/moduli; Mr and H_AC are swept below ---
H_BASE = 10.0                  # in
...
# Verified stable at iteration=5: max total depth here is 12+10=22in, well
# within the range already checked in sensitivity_AC_base_thickness_2D.py (up to 32in).
N = 40
H_AC_grid = np.linspace(3, 12, N)     # in
Mr_grid = np.linspace(4, 25, N)       # ksi

rows = []
Nf_grid = np.zeros((N, N))   # [i=Mr index, j=H_AC index]
Nd_grid = np.zeros((N, N))
life_grid = np.zeros((N, N))
gov_grid = np.zeros((N, N))  # 0 = fatigue-governed, 1 = rutting-governed

for i, Mr in enumerate(Mr_grid):
...
csv_path = os.path.join(out_dir, "Mr_AC_thickness_2D.csv")
...
# linear grids -> uniform half-step edges
dAC = (H_AC_grid[1] - H_AC_grid[0]) / 2
dMr = (Mr_grid[1] - Mr_grid[0]) / 2
AC_edges = np.concatenate([[H_AC_grid[0] - dAC], H_AC_grid[:-1] + dAC, [H_AC_grid[-1] + dAC]])
Mr_edges = np.concatenate([[Mr_grid[0] - dMr], Mr_grid[:-1] + dMr, [Mr_grid[-1] + dMr]])
...
ax1.pcolormesh(AC_edges, Mr_edges, gov_grid, cmap=gov_cmap, vmin=0, vmax=1, shading="flat")
ax1.contour(H_AC_grid, Mr_grid, np.log10(Nf_grid) - np.log10(Nd_grid),
...
ax1.set_ylabel("Subgrade resilient modulus, $M_r$ (ksi)", color=SEC_INK, fontsize=10)
...
legend_handles = [
    ...
    plt.Line2D([0], [0], marker="*", markersize=12, markerfacecolor=SURFACE,
               markeredgecolor=INK, linestyle="none", label="Baseline (6 in, 10 ksi)"),
]
...
mesh = ax2.pcolormesh(AC_edges, Mr_edges, np.log10(life_grid), cmap=seq_cmap, shading="flat")
ax2.contour(H_AC_grid, Mr_grid, np.log10(Nf_grid) - np.log10(Nd_grid),
...
ax2.set_ylabel("Subgrade resilient modulus, $M_r$ (ksi)", color=SEC_INK, fontsize=10)
...
fig.suptitle("Pavement Design Life — Subgrade $M_r$ x AC Thickness (2D sweep)\n"
             "(10 in base, $E_{AC}$=500 ksi, $E_{base}$=30 ksi)",
             color=INK, fontsize=13, fontweight="bold", y=0.99)

fig.tight_layout()
fig.subplots_adjust(top=0.82, wspace=0.32)
png_path = os.path.join(out_dir, "Mr_AC_thickness_2D.png")
